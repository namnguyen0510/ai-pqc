...
file_paths = [os.path.join('result_csv', x) for x in sorted(os.listdir('result_csv')) if '.DS_Store']
#print(file_paths[90:])

for i in tqdm.tqdm(range(234,len(file_paths))):
    if '01.' not in file_paths[i]:
        ...
        results = []
        model_types = ['Transformer', 'RNN', 'FNN', 'GRN', 'LSTM']
        num_epochs = 10
        ...
        for model_type in model_types:
            ...
            if model_type == 'RNN':
                model = RNN(input_size, 32, num_layers=2, output_size=output_size)  # Added num_layers
            elif model_type == 'FNN':
                model = FNN(input_size, 32, output_size)
            elif model_type == 'GRN':
                model = GRN(input_size, 32, output_size)
            elif model_type == 'LSTM':
                model = LSTM(input_size, 32, num_layers=2, output_size=output_size)  # Added num_layers
            elif model_type == 'Transformer':  
                model = TransformerModel(input_size, embed_size=32, num_heads=4, hidden_size=32, num_layers=2, output_size=output_size)
            else:
                    ...
                    for run in range(num_eval):
                        ...
                        for epoch in range(num_epochs):
                            ...
                            if val_loss < best_val_loss:
                                ...
                                best_model_path = f"/media/namng/Drive_1/pqc-crispr/results-pca-16/best-dnn/{os.path.basename(file_path)}_{model_type}-{optimizer_name}-{loss_name}-run{run+1}.pth"
        ...
        results_file = f'/media/namng/Drive_1/pqc-crispr/results-pca-16/{os.path.basename(file_path)}-result-DNN.csv'
